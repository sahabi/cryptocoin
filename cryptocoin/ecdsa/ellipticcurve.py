...
class CurveFp(object):
    """Elliptic Curve over the field of intergers modulo a prime."""
    ...
    def __repr__(self):
        return '{}({!r},{!r},{!r})'.format(self.__class__.__name__, self.__p,
                self.__a, self.__b)

    def __str__(self):
        return 'y^2 = x^3 + {}*x + {} (mod {})'.format(self.__a, self.__b,
                self.__p)


class Point(object):
    """A point on an elliptic curve. Altering x and y is forbidden, but they
    can be read by the x() and y() methods."""
    def __init__(self, curve, x, y, order=None):
        ...
        self.__y = y
        self.__order = order
        # self.curve is allowed to be None only for Inf:
        if self.__curve and not self.__curve.contains_point(x, y):
            raise NoSuchPointError('({},{}) is not on the curve {}'.format(x,
                y, curve))
        if order:
    ...
    def __add__(self, other):
        ...
        if self.__x == other.__x:
            if (self.__y + other.__y) % self.__curve.p() == 0:
                return INFINITY # i dont understand this
            else:
                return self.double()

        p = self.__curve.p()

        l = ((other.__y - self.__y) * numbertheory.inverse_mod(other.__x -
            self.__x, p)) % p
    ...
    def __repr__(self):
        return "{}({!r},{!r},{!r},{!r})".format(self.__class__.__name__,
                self.__curve, self.__x, self.__y, self.__order)
    ...
    def double(self):
        ...
        l = ((3 * self.__x * self.__x + a) * numbertheory.inverse_mod(2 *
            self.__y, p)) % p
        x3 = (l * l - 2 * self.__x) % p
        y3 = (l * (self.__x - x3) - self.__y) % p
